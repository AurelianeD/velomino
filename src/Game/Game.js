...
function Game(props) {
	...
	if (count === gamePreferences.numberOfRound) {
		playerList.sort((a, b) => b.finalScore - a.finalScore)
		return (
				<>
					<div className='flex flex-col gap-4 justify-center items-center md:flex-row'>
						{playerList.map((player, index) => {
									const hightScore = Math.max(...playerList.map(player => player.finalScore));
									return (
											<div key={index}>
												{hightScore === player.finalScore ?
														<div className="bg-white rounded-xl p-5 relative shadow-md">
															...
															<p className='text-xl text-purple'>{player.finalScore}</p>
														</div>
														:
														<div className="bg-white rounded-xl p-5 relative shadow-md">
															<p className="px-5 uppercase font font-bold text-xl">{player.name}</p>
